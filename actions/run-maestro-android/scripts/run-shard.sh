...
# second time. Compare canonicalized paths since PRE_RUN_FLOW and the
# discovered flow's path string may differ in form (relative vs
# "./"-prefixed, etc.) while resolving to the same file.
if [ -n "$PRE_RUN_FLOW" ]; then
  pre_run_flow_resolved=$(realpath -m "$PRE_RUN_FLOW")
  remaining_flows=()
  for flow in "${flows[@]}"; do
    if [ "$(realpath -m "$flow")" != "$pre_run_flow_resolved" ]; then
