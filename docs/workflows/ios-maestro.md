...
| `simulator-slim-profile` | no | `''` | Repository-relative path to a committed simslim JSON profile. When set, the booted simulator is checked with `simslim verify --profile` before the app is installed; any drift fails closed. See [actions/run-maestro-ios/README.md#slim-simulators-simslim](../../actions/run-maestro-ios/README.md#slim-simulators-simslim). |
