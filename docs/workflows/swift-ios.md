...
| `simulator-template-strategy` | `none`                  | `auto` clones the host's verified shut-down `mobile-ci-template-<device-type>-<runtime>` device when there is one, and otherwise leaves one behind once the lease is slim. Needs no knowledge of the host image. |
