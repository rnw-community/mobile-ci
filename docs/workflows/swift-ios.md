...
| `simulator-template-strategy` | `none`                  | `auto` clones the host's shut-down `mobile-ci-template-<device-type>-<runtime>` device when there is one — trusted by name, with the booted clone verified rather than the template — and otherwise leaves one behind once the lease is slim. Needs no knowledge of the host image. |
...
| `affected-tests-map`     | `''`                         | JSON map, resolved against `working-directory`, of path globs to test identifiers ([`xcodebuild-affected-tests`](../../actions/xcodebuild-affected-tests/README.md)). Set it and a **pull request** runs only the tests its changed files affect; a push, an unmapped file, a touched `affected-tests-full-suite-paths` glob or an unreachable base all run the full suite. Needs `checkout-fetch-depth: 0`, and the map has to be tracked by git (a change to it runs everything). When the selection widens back to `all`, the shard runs whatever `only-testing` says — the same list a push runs — so the two inputs never combine into a narrower run than a push would do. |
