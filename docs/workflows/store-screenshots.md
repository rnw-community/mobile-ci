...
| `capture-manifest`                  | **yes**  | —                                        | JSON array of capture matrix entries, one job per entry: `{"device": "iPhone 17 Pro Max", "locales": ["en","de"], "appearances": ["light","dark"], "orientation": "portrait"}`. `orientation` defaults to `"portrait"` when omitted. `device` must be unique across entries - `validate-manifest` fails closed on a duplicate, since two entries for the same device would slugify to the same `raw-screenshots-<device-slug>` artifact name, and `upload-artifact` requires unique names within a run. |
