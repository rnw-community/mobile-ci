...
| `capture-manifest`                  | **yes**  | —                                        | JSON array of capture matrix entries, one job per entry: `{"device": "iPhone 17 Pro Max", "locales": ["en","de"], "appearances": ["light","dark"], "orientation": "portrait"}`. `orientation` defaults to `"portrait"` when omitted. Every entry's `device`, slugified the same way `capture-screenshots-ios` does, must be unique - `validate-manifest` fails closed when two entries collide on the same slug (not just byte-identical `device` strings), since they would otherwise upload the same `raw-screenshots-<device-slug>` artifact name, and `upload-artifact` requires unique names within a run. |
