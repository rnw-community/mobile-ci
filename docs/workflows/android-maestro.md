...
| `flows-dir`                     | **yes**  | —                                                  | Directory (relative to repo root) whose top-level files are the runnable Maestro flows. Subdirectories are deliberately not searched; see `flows-name-pattern`. |
...
| `android-build-tools-dir`       | no       | `''`                                               | Path to the Android SDK build-tools directory, used by `repack-app` (its `--android-build-tools-dir` and `aapt2` validation) when `repack-on-hit` is true. Leave empty to rely on an `aapt2` already on `PATH`. |
